...
\times 32$ grid spanning the scene area. The grid is initialised with uniform probability. Each time a plant
...
\item The tangent-space normal is decoded from $[0,1]^3$ to $[-1,1]^3$ and transformed to world space using a
  Gram--Schmidt re-orthogonalised tangent frame ($T' = \text{normalize}(T - (T \cdot N)N)$, $B = N \times T'$).
